...
int main()
{
    ...
    while(true) // играть всегда
    {
        ...
        { // если игрок не перебрал
            ...
            if(all_score(comp_cards, n_comp) > 21)
            {
                printf("Банкир проиграл - перебор.\n");
            }
            // иначе, если у банкира сумма очков > чем у игрока, то
            // вывести сообщение, что игрок проиграл по очкам
            else if(all_score(comp_cards, n_comp) > all_score(player_cards, n_player))
            {
                printf("Игрок проиграл по очкам.\n");
            } 
            // иначе, если у банкира сумма очков < чем у игрока, то
            // вывести сообщение, что игрок проиграл по очкам
            else if(all_score(comp_cards, n_comp) < all_score(player_cards, n_player))
            {
                printf("Банкир проиграл по очкам.\n");
            }
